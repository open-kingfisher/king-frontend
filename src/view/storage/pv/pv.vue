...
import { getAllPV, deletePV, getOnePV, updatePV, createPV } from '@/api/storage'
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      getAllPV({ productId: 100 }).then(res => {
        let data = []
        forEach(res.data, (item, index) => {
          data.push({
            ...
            policy: item.spec.persistentVolumeReclaimPolicy || '-',
            claimRef:
