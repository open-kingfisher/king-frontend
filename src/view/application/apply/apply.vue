      ...
      <Col span="24">
        <Card dis-hover v-if="data1">
          ...
          <SearchTable :columns="columns" :data="data1">
            <ButtonGroup>
<!--              <Button v-if="hasPerm('add_dep')" @click="createBtnClick('CREATE')" style="margin-right: 3px" type="success">{{ this.$t('add') }}</Button>-->
<!--              <Button v-if="hasPerm('a_add_dep')" @click="advancedCreateBtnClick('CREATE')" style="margin-right: 3px" type="success" ghost>{{ this.$t('advanced_Add') }}</Button>-->
    ...
    <Row type="flex" justify="center" style="margin-bottom: 10px">
      <Col span="24">
        <Card dis-hover v-if="data2">
          ...
          <SearchTable :columns="columns" :data="data2"></SearchTable>
        </Card>
      </Col>
    </Row>
    <Row type="flex" justify="center">
      <Col span="24">
        <Card dis-hover v-if="data3">
          ...
          <SearchTable :columns="columns" :data="data3"></SearchTable>
        </Card>
      </Col>
    </Row>
    <Drawer
    ...
    <Modal v-model="deleteModel" width="360">
      ...
      <div style="text-align:center">
        <p style="line-height: 35px; margin-bottom: 10px">确认删除  <strong style="color:#f60;text-align:center">{{ctrl}}</strong> 吗？</p>
      </div>
      <div slot="footer">
...
<!--          <Step title="入口"></Step>-->
      ...
      <Form ref="createFormItem" v-show="current === 0" :model="formItemBase" :label-width="70" :rules="ruleValidate">
        ...
        <FormItem label="选择模板" v-if="templateDisplay">
          <Select v-model="formItemBase.template" placeholder="通过模板创建" @on-change="templateChange(formItemBase.template)">
        ...
        <FormItem style="margin-left: 60px" label="选择节点"  v-if="formItemBase.nodeDisplay === 1 && formItemBase.nodeSelectType === 'nodeLabels'">
           <Select v-model="formItemBase.node" placeholder="通过节点选取标签" @on-change="nodeChange(formItemBase.node)">
             <Option v-for="v in node" :value="v.name" :key="v.name">{{ v.name }}</Option>
           </Select>
        </FormItem>
        <div style="margin-left: 60px" v-for="(item, nodeSelectorIndex) in formItemBase.nodeSelector" :key="nodeSelectorIndex + 'nodeSelectorIndex'" :value="item" v-if="formItemBase.nodeDisplay === 1 && formItemBase.nodeSelectType === 'nodeLabels'">
          <FormItem :label="nodeSelectorIndex === 0 ? '节点标签': ''">
          <Row type="flex" justify="space-between">
            <Col span="11">
              <FormItem>
            ...
            <Col span="1" offset="0">
              <Button type="dashed" @click="handleRemove('formItemSelector', nodeSelectorIndex)" icon="md-trash">
              </Button>
            </Col>
            <Col span="1" offset="0">
        ...
        <FormItem style="margin-left: 60px" label="选择节点"  v-if="formItemBase.nodeDisplay === 1 && formItemBase.nodeSelectType === 'nodeAssign'">
        ...
        <div v-for="(item, labelsIndex) in formItemBase.labels" :key="labelsIndex + 'labels'" :value="item">
          <FormItem :label="labelsIndex === 0 ? 'Pod标签': ''" >
            <Row type="flex" justify="space-between">
              ...
              <Col span="1" offset="0">
                <Button type="dashed" @click="handleRemove('podLabels',labelsIndex)" :disabled= "formItemBase.labels.length === 1 || nameDisabled" icon="md-trash">
                </Button>
              </Col>
              <Col span="1" offset="0">
        ...
        <!--annotations-->
        <div v-for="(item, annotationsIndex) in formItemBase.annotations" :key="annotationsIndex + 'annotations'" :value="item">
          <FormItem :label="annotationsIndex === 0 ? 'Pod注解': ''">
            <Row type="flex" justify="space-between">
              ...
              <Col span="1" offset="0">
                <Button type="dashed" @click="handleRemove('podAnnotations',annotationsIndex)" :disabled="formItemBase.annotations.length === 1" icon="md-trash">
                </Button>
              </Col>
              <Col span="1" offset="0">
      ...
      <Form ref="createFormItemVolume" v-show="current === 1" :model="formItemVolume" :label-width="60">
        ...
        <div style="margin-left: 60px" v-for="(item, hostPathIndex) in formItemVolume.hostPath" :key="hostPathIndex + 'hostPath'" :value="item" v-if="formItemVolume.volumeDisplay === 1">
        ...
        <div style="margin-left: 60px" v-for="(item, secretIndex) in formItemVolume.secret" :key="secretIndex + 'secret'" :value="item" v-if="formItemVolume.volumeDisplay === 1">
        ...
        <div style="margin-left: 60px" v-for="(item, configMapIndex) in formItemVolume.configMap" :key="configMapIndex + 'configMap'" :value="item" v-if="formItemVolume.volumeDisplay === 1">
        ...
        -->
        <div style="margin-left: 60px" v-for="(item, cephFSIndex) in formItemVolume.cephFS" :key="cephFSIndex + 'cephFS'" :value="item" v-if="formItemVolume.volumeDisplay === 1">
        ...
        <div v-for="(item, index) in formItemConfig.containers" :key="index" :value="item">
          ...
          <FormItem class="ivu-form-item ivu-form-item-required" label="容器类型">
            <Row type="flex" justify="space-between">
              ...
              <Col span="1" offset="0" style="margin-right: 0px">
                <Button type="dashed" @click="handleRemove('formItemConfig', index)" :disabled= "formItemConfig.containers.length === 1" icon="md-trash">
                </Button>
              </Col>
            </Row>
          </FormItem>
          <!--容器名-->
          <FormItem class="ivu-form-item ivu-form-item-required" label="容器名">
            <Row type="flex" justify="space-between">
              <Col span="24">
                <FormItem
          ...
          <!--Port-->
          <FormItem label="容器端口">
            <i-switch size="large" v-model="formItemConfig.containers[index].portsDisplay" :true-value=1 :false-value=0 @on-change="handleAdd('portsAdd', index)">
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
          <div v-for="(item, portsIndex) in formItemConfig.containers[index].ports" :key="portsIndex + 'ports'" :value="portsIndex" v-if="formItemConfig.containers[index].portsDisplay === 1">
            <FormItem>
            <Row type="flex" justify="space-between">
              <Col span="6" offset="0" style="margin-right: 0px">
                <FormItem>
          ...
          <!--VolumeMount-->
          <FormItem label="挂载卷">
            <i-switch size="large" v-model="formItemConfig.containers[index].volumeMountDisplay" :true-value=1 :false-value=0 @on-change="handleAdd('volumeMountAdd', index)">
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
          <div v-for="(item, volumeMountIndex) in formItemConfig.containers[index].volumeMounts" :key="volumeMountIndex + 'volumeMount'" :value="volumeMountIndex" v-if="formItemConfig.containers[index].volumeMountDisplay === 1">
            <FormItem>
            <Row type="flex" justify="space-between">
          ...
          <!--资源配额-->
          <FormItem label="资源配额">
            <i-switch size="large" v-model="formItemConfig.containers[index].resourcesDisplay" :true-value=1 :false-value=0>
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
              ...
              <Col span="12">
                <FormItem
                  ...
                  <Input v-model="formItemConfig.containers[index].resources.requests.cpu">
                    ...
                    <Select v-model="formItemConfig.containers[index].resources.requests.cpuUnit" slot="append" style="width: 70px">
                ...
                <FormItem
                  ...
                  :rules="{required: false, message: '必须是非零数字', pattern:/^\+?[1-9][0-9]*$/ , trigger: 'blur'}">
                   <Input v-model="formItemConfig.containers[index].resources.limits.cpu">
                     ...
                     <Select v-model="formItemConfig.containers[index].resources.limits.cpuUnit" slot="append" style="width: 70px">
                      <Option value="m">m</Option>
                      <Option value="core">core</Option>
                    </Select>
                  </Input>
              ...
              <Col span="12">
                <FormItem
                  ...
                  <Input v-model="formItemConfig.containers[index].resources.requests.memory">
                    <span slot="prepend">最小要求</span>
                    <Select v-model="formItemConfig.containers[index].resources.requests.memoryUnit" slot="append" style="width: 70px">
                 ...
                 <FormItem
                  ...
                  :rules="{required: false, message: '必须是非零数字', pattern:/^\+?[1-9][0-9]*$/ , trigger: 'blur'}">
                   <Input v-model="formItemConfig.containers[index].resources.limits.memory">
                     <span slot="prepend">最大限制</span>
                     <Select v-model="formItemConfig.containers[index].resources.limits.memoryUnit" slot="append" style="width: 70px">
                      <Option value="Ki">Ki</Option>
                      <Option value="Mi">Mi</Option>
                      <Option value="Gi">Gi</Option>
                      <Option value="Ti">Ti</Option>
          ...
          <FormItem label="命令与参数">
            <i-switch size="large" v-model="formItemConfig.containers[index].commandDisplay" :true-value=1 :false-value=0>
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
          ...
          <!--命令-->
          <div style="margin-left: 40px"  v-for="(item, commandIndex) in formItemConfig.containers[index].command" :key="commandIndex + 'command'" :value="item.key">
            <FormItem :label="commandIndex === 0 ? '命令': ''" v-if="formItemConfig.containers[index].commandDisplay === 1">
              <Row type="flex" justify="space-between">
                <Col span="22">
                  <FormItem>
          ...
          <!--参数-->
          <div style="margin-left: 40px"  v-for="(item, argsIndex) in formItemConfig.containers[index].args" :key="argsIndex + 'args'" :value="item.key">
          ...
          <!--手动输入环境变量-->
          <div style="margin-left: 60px" v-for="(item, envIndex) in formItemConfig.containers[index].env" :key="envIndex + 'env'" :value="item">
          ...
          <!--通过configMap获取-->
          <div style="margin-left: 60px" v-for="(item, envFromConfigMapIndex) in formItemConfig.containers[index].envFromConfigMap" :key="envFromConfigMapIndex + 'envFromConfigMap'" :value="item">
            <FormItem v-if="formItemConfig.containers[index].envDisplay === 1" :label="envFromConfigMapIndex === 0 ? '配置字典': ''">
              <Row type="flex" justify="space-between">
                <Col span="7">
                  <FormItem>
                    <Input v-model="item.name">
                ...
                <Col span="8">
                  <FormItem>
                    <Select v-model="item.valueFrom.name" placeholder="资源" @on-change="getKeyByConfigMap(item.valueFrom.name)">
                      <Option v-for="(item, index) in configMap" :value="item.name" :key="index" >{{ item.name }}</Option>
                    </Select>
                  </FormItem>
                </Col>
          ...
          <!--通过secret获取-->
          <div style="margin-left: 60px" v-for="(item, envFromSecretIndex) in formItemConfig.containers[index].envFromSecret" :key="envFromSecretIndex + 'envFromSecret'" :value="item">
            <FormItem v-if="formItemConfig.containers[index].envDisplay === 1" :label="envFromSecretIndex === 0 ? '保密字典': ''">
              <Row type="flex" justify="space-between">
                <Col span="7">
                  <FormItem>
                    <Input v-model="item.name">
                ...
                <Col span="8">
                  <FormItem>
                    <Select v-model="item.valueFrom.name" placeholder="资源" @on-change="getKeyBySecret(item.valueFrom.name)">
                      <Option v-for="(item, index) in secret" :value="item.name" :key="index" >{{ item.name }}</Option>
                    </Select>
                  </FormItem>
                </Col>
          ...
          <!--field获取-->
          <div style="margin-left: 60px" v-for="(item, envFromFieldIndex) in formItemConfig.containers[index].envFromField" :key="envFromFieldIndex + 'envFromField'" :value="item">
            <FormItem v-if="formItemConfig.containers[index].envDisplay === 1" :label="envFromFieldIndex === 0 ? '描述字段': ''">
              <Row type="flex" justify="space-between">
                <Col span="7">
                  <FormItem>
                    <Input v-model="item.name">
          ...
          <!--存活探针-->
          <FormItem label="存活探针">
            <i-switch size="large" v-model="formItemConfig.containers[index].livenessProbeDisplay" :true-value=1 :false-value=0>
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
          ...
          <!-- httpGet-->
          <FormItem v-if="formItemConfig.containers[index].livenessProbeType === 'httpGet' && formItemConfig.containers[index].livenessProbeDisplay === 1">
            <Row type="flex" justify="space-between">
              <Col span="3">
                <FormItem>
          ...
          <!-- command-->
          <div v-for="(item, livenessProbeCommandIndex) in formItemConfig.containers[index].livenessProbeCommand.exec.command" :key="livenessProbeCommandIndex + 'livenessProbe'" :value="item">
              <FormItem v-if="formItemConfig.containers[index].livenessProbeType === 'command' && formItemConfig.containers[index].livenessProbeDisplay === 1">
              <Row type="flex" justify="space-between">
                <Col span="21">
                  <FormItem
                ...
                <Col span="1" offset="0" style="margin-right: 0px">
                  <Button type="dashed" @click="handleRemove('livenessProbeCommand',index, livenessProbeCommandIndex)" :disabled= "formItemConfig.containers[index].livenessProbeCommand.exec.command.length === 1" icon="md-trash">
                  </Button>
                </Col>
                <Col span="1" offset="0" style="margin-right: 0px">
          ...
          <!-- socket-->
          <FormItem v-if="formItemConfig.containers[index].livenessProbeType === 'socket' && formItemConfig.containers[index].livenessProbeDisplay === 1">
            <Row type="flex" justify="space-between">
              <Col span="24">
                <FormItem
          ...
          <!--就绪探针-->
          <FormItem label="就绪探针">
            <i-switch size="large" v-model="formItemConfig.containers[index].readinessProbeDisplay" :true-value=1 :false-value=0>
              <span slot="open">开启</span>
              <span slot="close">关闭</span>
            </i-switch>
          </FormItem>
          ...
          <!-- httpGet-->
          <FormItem v-if="formItemConfig.containers[index].readinessProbeType === 'httpGet' && formItemConfig.containers[index].readinessProbeDisplay === 1">
            <Row type="flex" justify="space-between">
              <Col span="3">
                <FormItem>
          ...
          <!-- command-->
          <div v-for="(item, readinessProbeCommandIndex) in formItemConfig.containers[index].readinessProbeCommand.exec.command" :key="readinessProbeCommandIndex + 'readinessProbe'" :value="item">
            <FormItem v-if="formItemConfig.containers[index].readinessProbeType === 'command' && formItemConfig.containers[index].readinessProbeDisplay === 1">
              <Row type="flex" justify="space-between">
                <Col span="21">
                  <FormItem
                ...
                <Col span="1" offset="0" style="margin-right: 0px">
                  <Button type="dashed" @click="handleRemove('readinessProbeCommand',index, readinessProbeCommandIndex)" :disabled= "formItemConfig.containers[index].readinessProbeCommand.exec.command.length === 1" icon="md-trash">
                  </Button>
                </Col>
                <Col span="1" offset="0" style="margin-right: 0px">
          ...
          <!-- socket-->
          <FormItem v-if="formItemConfig.containers[index].readinessProbeType === 'socket' && formItemConfig.containers[index].readinessProbeDisplay === 1">
            <Row type="flex" justify="space-between">
              <Col span="24">
                <FormItem
      ...
      <Form ref="createFormAnnotations" v-show="current === 3" :model="formItemAnnotations" :label-width="55">
        <div v-for="(item, index) in formItemAnnotations.labels" :key="index + 'labels'" :value="item">
          ...
          <Row type="flex" justify="space-between">
            ...
            <Col span="1" offset="0" style="margin-right: 0px">
              <Button type="dashed" @click="handleRemove('labels',index)" :disabled= "formItemAnnotations.labels.length === 1" icon="md-trash">
              </Button>
            </Col>
          </Row>
          </FormItem>
        ...
        <div v-for="(item, index) in formItemAnnotations.items" :key="index + 'annotations'" :value="item">
          ...
          <Row type="flex" justify="space-between">
            ...
            <Col span="1" offset="0" style="margin-right: 0px">
              <Button type="dashed" @click="handleRemove('annotations',index)" :disabled= "formItemAnnotations.items.length === 1" icon="md-trash">
              </Button>
            </Col>
          </Row>
          </FormItem>
  ...
  methods: {
      ...
      getControllerInfo({
        ...
        res.data.forEach((item, index) => {
          ...
          data.push({
            ...
            conPercent: (item.status.readyReplicas || '0') + ' / ' + (item.status.replicas || '0'),
    ...
    handleRemove (type, index, secondIndex) {
      ...
      if (type === 'formItemSelector') {
        this.formItemBase.nodeSelector.splice(index, 1)
        if (this.formItemBase.nodeSelector.length === 0) {
          this.formItemBase.node = ''
        }
      }
    ...
    formOption (value) {
      ...
      if (this.formItemBase.nodeDisplay === 1 && this.formItemBase.nodeSelectType === 'nodeLabels') {
        this.formItemBase.nodeSelector.forEach((item, index) => {
          if (item.key) {
            json.spec.template.spec.nodeSelector[item.key] = item.val
          }
        })
      }
      ...
      this.formItemConfig.containers.forEach((item, index) => {
        ...
        if (item.livenessProbeDisplay === 1) {
          if (item.livenessProbeType === 'httpGet') {
            livenessProbe = {
              'httpGet': {
                ...
                'port': parseInt(item.livenessProbeHTTPGet.httpGet.port),
          ...
          if (item.livenessProbeType === 'socket') {
            livenessProbe = {
              'tcpSocket': {
                'port': parseInt(item.livenessProbeTcpSocket.tcpSocket.port)
              },
        ...
        if (item.readinessProbeDisplay === 1) {
          if (item.readinessProbeType === 'httpGet') {
            readinessProbe = {
              'httpGet': {
                ...
                'port': parseInt(item.readinessProbeHTTPGet.httpGet.port),
          ...
          if (item.readinessProbeType === 'socket') {
            readinessProbe = {
              'tcpSocket': {
                'port': parseInt(item.readinessProbeTcpSocket.tcpSocket.port)
              },
        ...
        res.data.spec.template.spec.volumes.forEach((item, index) => {
          if (item.configMap) {
            this.formItemVolume.configMap.push({
              ...
              defaultMode: item.configMap.defaultMode
            })
          }
          if (item.secret) {
            this.formItemVolume.secret.push({
              ...
              defaultMode: item.secret.defaultMode
            })
          }
          if (item.hostPath) {
            this.formItemVolume.hostPath.push({
