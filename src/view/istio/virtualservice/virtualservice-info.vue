...
import { Get, Chart } from '@/api/istio/virtualservices.js'
...
export default {
  ...
  mounted () {
    ...
    if (this.$route.params.resourceName) {
      ...
      localStorage.setItem('virtualServiceName', this.resourceName)
    } else {
      this.resourceName = localStorage.getItem('virtualServiceName')
    }
